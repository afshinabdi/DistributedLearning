...
import distributed_data.sg_quantizers.tf_implementation.dithered_transform_quantizer as tf_dqt
...
class BaseModel:
    ...
    def create_network(self, parameters: dict):
        seed = parameters.get('seed', _DEFAULT_SEED)
    ...
    def _add_gradient_quantizers(self, parameters):
        ...
        self._gWh = [0] * self._num_layers
        self._qgW = [0] * self._num_layers
        self._sgW = [0] * self._num_layers

        self._gbh = [0] * self._num_layers
        self._qgb = [0] * self._num_layers
        self._sgb = [0] * self._num_layers

        # operators to update the quantization reside (if necessary)
        self._updateRw = [tf.no_op()] * self._num_layers
        self._updateRb = [tf.no_op()] * self._num_layers

        if quantization_method == '':
            self._qgW, self._qgb = self._gW, self._gb
            self._gWh, self._gbh = self._gW, self._gb

        elif quantization_method == 'dqsg':
            ...
            for layer in range(self._num_layers):
                # weight of the layer
                self._qgW[layer], self._sgW[layer], self._gWh[layer] = tf_dq.quantize(
                    self._gW[layer], num_levels, bucket_sizes[layer][0], seeds[2 * layer]
                )

                # bias of the layer
                self._qgb[layer], self._sgb[layer], self._gbh[layer] = tf_dq.quantize(
                    self._gb[layer], num_levels, bucket_sizes[layer][1], seeds[2 * layer + 1]
                )

        elif quantization_method == 'dqtsg':
            bucket_sizes = parameters.get('bucket_sizes')
            num_levels = parameters.get('num_levels')
            ...
            for layer in range(self._num_layers):
                # weight of the layer
                self._qgW[layer], self._sgW[layer], self._gWh[layer] = tf_dqt.quantize(
                    self._gW[layer], H_matrices[layer][0], num_levels, bucket_sizes[layer][0], seeds[2 * layer]
                )

                # bias of the layer
                self._qgb[layer], self._sgb[layer], self._gbh[layer] = tf_dqt.quantize(
                    self._gb[layer], H_matrices[layer][1], num_levels, bucket_sizes[layer][1], seeds[2 * layer + 1]
                )
            ...
            for layer in range(self._num_layers):
                # weight of the layer
                self._qgW[layer], self._sgW[layer], self._gWh[layer] = tf_qsg.quantize(
                    self._gW[layer], num_levels, bucket_sizes[layer][0]
                )

                # bias of the layer
                self._qgb[layer], self._sgb[layer], self._gbh[layer] = tf_qsg.quantize(
                    self._gb[layer], num_levels, bucket_sizes[layer][1]
                )

        elif quantization_method == 'csq':
            num_levels = parameters.get('num_levels')
            ...
            for layer in range(self._num_layers):
                # weight of the layer
                self._qgW[layer], self._sgW[layer], self._gWh[layer], self._updateRw[layer], _ = tf_csq.quantize(
                    self._gW[layer], T_matrices[layer][0], num_levels, seeds[2 * layer], use_Rademacher, shuffle_T,
                    err_feedback
                )

                # bias of the layer
                self._qgb[layer], self._sgb[layer], self._gbh[layer], self._updateRb[layer], _ = tf_csq.quantize(
                    self._gb[layer], T_matrices[layer][1], num_levels, seeds[2 * layer + 1], use_Rademacher, shuffle_T,
                    err_feedback
                )
    ...
    def quantized_gradients(self, x, y, drop_rate=0.):
        assert self._qgW is not None, 'The operators to quantize the gradients have not been defined.'

        return self._sess.run(
            [self._qgW, self._sgW, self._qgb, self._sgb, self._updateRw, self._updateRb],
            feed_dict={
                self._input: x,
                self._target: y,
                self._drop_rate: drop_rate
    ...
    # _________________________________________________________________________
    # Get the dequantized gradients of the parameters of the NN for the given input
    def dequantized_gradients(self, x, y, drop_rate=0.):
        assert self._qgW is not None, 'The operators to quantize the gradients have not been defined.'

        return self._sess.run(
            [self._gWh, self._gbh, self._qgW, self._sgW, self._qgb, self._sgb, self._updateRw, self._updateRb],
            feed_dict={
                self._input: x,
                self._target: y,
                self._drop_rate: drop_rate,
