...
class LenetModel:
    ...
    def add_regulizer(self, l1_weight=None, l2_weight=None):
        if self._loss is None:
            raise ValueError('The network has not been defined yet. First build the network, then add the regularizer.')
        ...
        if len(l1_weight) != num_parameters or len(l2_weight) != num_parameters:
            raise ValueError('Number of weights for the l1/l2 regularization is not the same as that of parameters.')
    ...
    def create_optimizer(self, training_algorithm='Adam', learning_rate=0.01, decay_rate=0.95, decay_step=100):
        with self._graph.as_default():
            ...
            self._trainOp = self._optimizer.minimize(self._loss, var_list=var_list, global_step=train_counter)
